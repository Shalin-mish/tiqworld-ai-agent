...
// classify() returns { type, confidence, scores } — helper unwraps .type
...
describe('classify() — returns object shape', () => {
  it('has type, confidence, scores fields', () => {
    const result = classify('fix the login bug');
    expect(result).toHaveProperty('type');
    expect(result).toHaveProperty('confidence');
    expect(result).toHaveProperty('scores');
  });

  it('confidence is 0 for unrecognised input', () => {
  ...
  it('confidence is between 0 and 100', () => {
    const result = classify('fix the login bug and trace the error');
...
describe('getTools()', () => {
  ...
  it('feature and maintenance share identical tool sets', () => {
    ...
    expect(maintenance).toBe(feature);
  });
