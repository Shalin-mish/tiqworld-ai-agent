...
function showDiff(oldContent, newContent) {
  ...
  for (let i = 0; i < maxLen; i++) {
    ...
    else if (oldLine !== newLine) { diff.push(`- ${oldLine}`); diff.push(`+ ${newLine}`); }
  }
  return diff.join('\n');
}
...
function gitCommit(fullPath, reason) {
  try {
    ...
    execSync(`git add "${relPath}" && git commit -m "agent: ${reason}"`,
...
export async function writeFile({ file_path, new_content, reason, _user = 'unknown' }) {
  try {
    ...
    if (isNewFile) {
      ...
      if (!diff) return { status: 'skipped', reason: 'No changes detected — file already matches.' };
    ...
    if (answer !== 'yes' && answer !== 'y') {
      return { status: 'rejected', file_path, message: 'User rejected the change. No file was modified.' };
    }

    let backup = { success: false };
    if (!isNewFile) backup = gitCommit(fullPath, reason);
    ...
    return { status: 'success', file_path, is_new_file: isNewFile, backup, archive: archiveFile,
      message: `File written successfully: ${file_path}` };
  } catch (err) {
    return { error: err.message, suggestion: 'Check if the path is valid and you have write permissions' };
  }
}
